...
shoplistfile = '/home/uav/my_biue/UAV-path-planning-main/Single UAV path planning/path planning/SAC'  # 保存训练结果
shoplistfile_test = '/home/uav/my_biue/UAV-path-planning-main/Single UAV path planning/path planning/SAC_indextest'  # 保存测试结果
...
class Actor():
    def __init__(self):
        self.action_net=ActorNet(state_number,action_number).to(device)
    ...
    def choose_action(self,s):
        ...
        return action.cpu().detach().numpy()  # 返回CPU上的numpy数组供环境使用

    def evaluate(self,s):
        mean,std=self.action_net(s)
        dist = torch.distributions.Normal(mean, std)
        noise = torch.distributions.Normal(0, 1)
        z = noise.sample().to(device)  # 确保噪声在正确设备上
...
class Entroy():
    def __init__(self):
        ...
        self.log_alpha = torch.zeros(1, requires_grad=True, device=device)  # 直接在目标设备上创建
...
class Critic():
    def __init__(self):
        self.critic_v = CriticNet(state_number,action_number).to(device)
        self.target_critic_v = CriticNet(state_number,action_number).to(device)
...
def run(env):
    if Switch==0:
        ...
        for k in range(TRAIN_NUM):
            ...
            for episode in range(EP_MAX):
                ...
                for timestep in range(EP_LEN):
                    ...
                    observation_, reward,done,edge_r,obstacle_r,goal_r,win= env.step(action)
                ...
                print(f"Ep: {episode} rewards: {reward_totle}")
                ...
                if episode % 20 == 0 and episode > 200:
                    # 保存时确保模型在CPU上，提高兼容性
                    save_data = {
                        'net': {k: v.cpu() for k, v in actor.action_net.state_dict().items()},
                        'opt': actor.optimizer.state_dict(), 
                        'i': episode
                    }
                    torch.save(save_data, "./Path_SAC_actor.pth")
        ...
        d = {
            "all_ep_r_mean": all_ep_r_mean, "all_ep_r_std": all_ep_r_std,
        ...
        plt.fill_between(np.arange(len(all_ep_edge_mean)), all_ep_edge_max, all_ep_edge_min, alpha=0.6, facecolor='#e75840')
        plt.xlabel('Episode')
        ...
        aa = Actor()
        # 加载模型时自动适应当前设备
        checkpoint_aa = torch.load("./Path_SAC_actor.pth", map_location=device)
        aa.action_net.load_state_dict(checkpoint_aa['net'])
        aa.action_net.to(device)  # 确保模型在正确设备上
        ...
        for j in range(TEST_EPIOSDE):
            ...
            for timestep in range(EP_LEN):
                ...
                new_state, reward,done ,edge_r,obstacle_r,goal_r,win= env.step(action)
        ...
        print(f'任务完成率: {win_times / TEST_EPIOSDE}')
        print(f'平均最短飞行时间: {average_timestep / TEST_EPIOSDE}')
        print(f'平均最短飞行路程: {average_integral_V / TEST_EPIOSDE}')
        print(f'平均最小能量损耗: {average_integral_U / TEST_EPIOSDE}')
        plt.show()
