...
def get_resource_path(subpath=''):
    """
    返回资源文件的绝对路径，自动适配 Linux / Windows
    subpath: 相对资源路径（相对 assignment 目录）
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))  # 当前文件所在目录
    resource_dir = os.path.join(base_dir, 'source')        # 默认资源根目录
    return os.path.join(resource_dir, subpath)
